...
//------------------------------------------------------------------------------------------
                                                                                     //작동 코드
void operate() {
  if ((saftyState == true && operateState == true) && stackCount > 0) {              //리미트 스위치가 닫혀있고, operateState 신호가 오면 
    stackCount--;                                                                    //쌓여있던 시간을 1씩 감소하면서
    turnBase();                                                                      //서보모터 작동
    delay(900);                                                                      //0.9초씩 작동 loop에 0.1초를 더해서 1초씩 감소하게됨
    if (stackCount == 0)
...
//문이 열려있는지 판단하는 코드
void opendoor() {
  if(saftyState == false ){                             //리미트 스위치 꺼져있으면
    digitalWrite(light , HIGH);                         //내부 LED를 키는 코드
    lightState = true;
  }
  else{                                                 //문을 닫으면 LED를 끄는 코드
    digitalWrite(light , LOW);
    lightState = false;
  }
}

//문을 닫은 상태로 작동시키면 내부 LED를 키는 코드
void closedoor() {                                    
  if(saftyState == true && turnState == true)           //안전 스위치와 모터가 작동하면 
    digitalWrite(light , HIGH);                         //LED를 키는 코드
}
